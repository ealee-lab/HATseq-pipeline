...
def set_sample_names(sample_df):
	...
	if "tissue" in colnames and "rep" in colnames:
		sample_df.loc[:, "sample_name"] = sample_df["donor"] + "-" + sample_df["tissue"] + "_" + sample_df["rep"]
...
def get_read_group(wildcards):
	...
	rg = fr"@RG\tID:{te}\tSM:{wildcards.sample}\tPL:{pl}\tLB:HAT-seq"
	return rg
...
def get_qc_runtime(wildcards, input, attempt):
	minutes = (int(input.size_mb / 400) + 5) * (2 ** (attempt - 1))
	return minutes

def get_preprocessing_runtime(wildcards, input, attempt):
	try:
		minutes = int(input.size_mb / 200) * (2 ** (attempt - 1))
