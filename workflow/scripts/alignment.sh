...
cat <(samtools view -h -F 16 -L <(awk '{if($6=="+") print}' "${snakemake_output[trim_peaks]}") "${snakemake_output[bwa_sorted_bam]}") \
	<(samtools view -f 16 -L <(awk '{if($6=="-") print}' "${snakemake_output[trim_peaks]}") "${snakemake_output[bwa_sorted_bam]}") | 
	samtools view -b - \
